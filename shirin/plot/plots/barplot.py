...
class BarPlot(AbstractPlot):
    ...
    def draw(self, data: pd.DataFrame) -> Any:
        ...
        if (
            ...
            and self.options.hue is not None
            and isinstance(self._palette, dict)
        ):
            return self._draw_stacked(data)

        if self.options.orientation == 'vertical':
            return self.renderer.render_barplot(
                df=data,
    ...
    def _draw_stacked(self, data: pd.DataFrame) -> Any:
        df_prepared = prepare_stacked_data(
            data,
            self.options.hue,  # type: ignore[arg-type]
            self.options.axis_column,
        ...
        colors = create_colors_list(df_prepared, self._palette)  # type: ignore[arg-type]
    ...
    def format_plot(self, plot: Any) -> None:
        if (
            self.options.label_map is None
            and self.options.plot_legend
            and self.options.hue is not None
        ):
            self.options.label_map = create_default_label_map(
                self._preprocessed_df,  # type: ignore[arg-type]
                self.options.hue,
            )
